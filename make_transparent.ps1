...
$srcPath = 'assets\images\chocolate.jpg'
$dstPath = 'assets\images\chocolate.png'
...
# Process pixels: pure white / off-white background become transparent
for ($i = 0; $i -lt $bytes; $i += 4) {
    ...
    if ($minVal -ge 245 -and $maxDiff -le 10) {
        ...
        $dstRgb[$i + 2] = 0
        $dstRgb[$i + 3] = 0
    } elseif ($minVal -ge 230 -and $maxDiff -le 15) {
        ...
        # Opaque foreground (ice cream & waffle cone)
        $dstRgb[$i] = [byte]$b
        $dstRgb[$i + 1] = [byte]$g
        $dstRgb[$i + 2] = [byte]$r
        $dstRgb[$i + 3] = 255
...
Write-Output 'Converted to transparent PNG successfully!'
